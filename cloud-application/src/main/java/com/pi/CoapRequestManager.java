...
public class CoapRequestManager {
    ...
    public void sendBatteryCommand(int floor, String setpoint) {
        String ip = FloorManager.getDeviceIP(floor, "battery");
        if (ip == null) {
            logger.error("No battery device found for floor {}", floor);
            System.err.println("No battery device found for floor " + floor);
            return;
        }
        
        String url = String.format("coap://[%s]:5683/battery/setpoint?setpoint=%s", ip, setpoint);
        logger.info("Sending Battery command to floor {}: setpoint={}", floor, setpoint);
        sendRequest(url, "Battery (Floor " + floor + ")");
    }
