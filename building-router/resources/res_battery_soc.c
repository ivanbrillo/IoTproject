...
static void res_get_handler(coap_message_t *request, coap_message_t *response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset)
{
  ...
  char buff[10];
  float_to_string(buff, current_soc);

  coap_set_payload(response, buffer, snprintf((char *)buffer, preferred_size, "{\"soc\": %s, \"v\": %ld}", buff, event_counter));
  event_counter++;
}
